...
test('AnimationClip', function () {
    var clip = new cc.AnimationClip();

    var curves;

    curves = clip.addProperty('a');
    curves.push({ frame: 0, value: 0 });

    curves = clip.addProperty('b', 'comp-01');
    curves.push({ frame: 1, value: 10 });

    curves = clip.addProperty('c', null, 'foo');
    curves.push({ frame: 2, value: 20 });

    curves = clip.addProperty('d', 'comp-02', 'bar');
    curves.push({ frame: 3, value: 30 });

    deepEqual(clip.curveData, {
        props: {
            a: [
                { frame: 0, value: 0 }
            ]
    ...
    }, 'clip should add curves');

    clip.removeProperty('c', null, 'foo');
    clip.removeProperty('d', null, 'foo');
    clip.removeProperty('a');

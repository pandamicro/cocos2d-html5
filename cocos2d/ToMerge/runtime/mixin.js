...
function mixin (node, typeOrTypename) {
    ...
    if (typeof typeOrTypename === 'string') {
        ...
        if ( !classToMix ) {
            cc.error('Fire.mixin: Failed to get class "%s"');
            if (cc._RFpeek()) {
        ...
        if ( !typeOrTypename ) {
            cc.error('Fire.mixin: The class to mixin must be non-nil');
        }
    ...
    if (CC_EDITOR) {
        ...
        if (!cc.FireClass.isFireClass(classToMix)) {
            cc.error('Fire.mixin: The class to mixin must be FireClass.');
            return;
        }
        if (!JS._getClassId(classToMix, false) && !CC_TEST) {
            cc.error("Fire.mixin: The class to mixin must have class name or script's uuid.");
            return;
        }
        if (!cc.isChildClassOf(classToMix, Behavior)) {
            cc.warn("Fire.mixin: The class to mixin must inherit from Fire.Behavior.");
            return;
        }
    }
    ...
    if (!node) {
        cc.error("Fire.mixin: The node to mixin must be non-nil.");
        return;
    }

    if (CC_EDITOR && node._mixinClasses && node._mixinClasses.indexOf(classToMix) !== -1) {
        cc.warn("Fire.mixin: The class has already mixined.");
        return;
    }
    ...
    for (var propName in classToMixProto) {
        ...
        if (isLifecycleMethods) {
            //if (Fire.engine && !Fire.engine._isPlaying && CC_EDITOR) {
            //    continue;
            //}
            scriptCtx[propName] = classToMixProto[propName];
            if (! lcmInitStates[lcmIndex]) {
                lcmInitStates[lcmIndex] = true;
                // cc.warn("Fire.mixin: %s's %s is overridden", cc(node).name, propName);
                (function () {
    ...
    if (Fire.engine && (Fire.engine._isPlaying || !CC_EDITOR) && !Fire.engine._isCloning) {
        // invoke onLoad
...
var exports = {
    ...
    hasMixin: function (node, typeOrTypename) {
        ...
        if (mixinClasses) {
            ...
            if (typeof typeOrTypename === 'string') {
                classToMix = JS.getClassByName(typeOrTypename);
                if ( !classToMix ) {
                    cc.error('Fire.hasMixin: Failed to get class "%s"', typeOrTypename);
                    return false;
                }
            }
            else {
    ...
    unMixin: function (node, typeOrTypename) {
        if ((Fire.engine && Fire.engine.isPlaying) || !CC_EDITOR) {
            return cc.warn("Fire.unMixin: Sorry, can not un-mixin when the engine is playing.");
        }
        ...
        if (!node) {
            return cc.error("Fire.unMixin: The node to un-mixin must be non-nil.");
        }
        ...
        if (mixinClasses) {
            ...
            if (typeof typeOrTypename === 'string') {
                ...
                if ( !classToUnmix ) {
                    return cc.error('Fire.unMixin: Failed to get class "%s"', typeOrTypename);
                }
            }
            else {
                if ( !typeOrTypename ) {
                    return cc.error('Fire.unMixin: The class to un-mixin must be non-nil');
                }
        ...
        return cc.error('Fire.unMixin: Can not find mixed class "%s" in node "%s".',
